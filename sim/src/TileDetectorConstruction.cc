...
G4VPhysicalVolume* TileDetectorConstruction::Construct(){
   ...
   G4double TileGapX            = 0*mm;
   G4double TileGapY            = 0*mm;
   ...
   G4VSolid* TileDetectorShape =
     new G4Box("logicalTileDetectorShape", (TileDimX+TileGapX) * nTilesX + TileGapX + 5*mm, (TileDimY+TileGapY) * nTilesY + TileGapY + 5*mm, TileDimZ); //its size
  ...
  //  Tiles
  //
   G4VSolid* TileShape = new G4Box("tileShape", TileDimX, TileDimY, TileDimZ);
   ...
   for(int i=0; i < nTilesX; i++){
     for(int j=0; j < nTilesY; j++){
       posCurrent =  G4ThreeVector((TileDimX+TileGapX) * nTilesX - (TileDimX+TileGapX) * 2*i,
                                   (TileDimY+TileGapY) * nTilesY - (TileDimY+TileGapY) * 2*j,
                                   0);
